...
def inputvalidator(input_="ohlc"):
    def dfcheck(func):
        ...
        def wrap(*args, **kwargs):
            ...
            # Map lại các cột theo chuẩn từ BarData sang 'open', 'high', 'low', 'close'
            rename_map = {
                "open_price": "open",
                "high_price": "high",
                "low_price": "low",
            ...
            df.columns = [c.lower() for c in df.columns]  # lowercase toàn bộ cột để thống nhất

            args[i] = df

            # Xác định cột 'close' theo người dùng (nếu có)
            inputs = {
                "o": "open",
                "h": "high",
                "l": "low",
...
class smc:
    ...
    def bos_choch(
        ...
        """
        BOS - Break of Structure với điều kiện Inducement đầy đủ
        CHoCH - Change of Character với điều kiện Inducement đầy đủ

        Inducement Logic:
        - Bullish BOS: Phải có Lower Low (liquidity grab) trước khi break Higher High
        - Bearish BOS: Phải có Higher High (liquidity grab) trước khi break Lower Low
        - CHoCH: Tương tự nhưng không cần break ATH/ATL, chỉ cần break gần nhất

        parameters:
        swing_highs_lows: DataFrame - provide the dataframe from the swing_highs_lows function
        close_break: bool - if True then the break will be based on close, otherwise high/low

        returns:
        BOS = 1 if bullish BOS, -1 if bearish BOS
        CHOCH = 1 if bullish CHoCH, -1 if bearish CHoCH
        Level = the level that was broken
        BrokenIndex = the index where the break occurred
        InducementLevel = the level of inducement (liquidity grab)
        InducementIndex = the index where inducement occurred
        """
        ...
        inducement_level = np.zeros(len(ohlc), dtype=np.float32)
        inducement_index = np.zeros(len(ohlc), dtype=np.int32)
        ...
        for i in range(len(swing_highs_lows)):
            if not np.isnan(swing_highs_lows["HighLow"].iloc[i]):
                swing_points.append({
                    'index': i,
                    'type': int(swing_highs_lows["HighLow"].iloc[i]),  # 1 for high, -1 for low
                    'level': swing_highs_lows["Level"].iloc[i]
                })

        def detect_current_trend(points, current_idx):
            """Xác định trend hiện tại dựa trên swing points gần nhất"""
            if len(points) < 4:
                return None

            recent_points = points[-4:]
            highs = [p for p in recent_points if p['type'] == 1]
            lows = [p for p in recent_points if p['type'] == -1]

            if len(highs) >= 2 and len(lows) >= 2:
                # Uptrend: Higher Highs and Higher Lows
                if (highs[-1]['level'] > highs[-2]['level'] and
                        lows[-1]['level'] > lows[-2]['level']):
                    return 'uptrend'
                # Downtrend: Lower Highs and Lower Lows
                elif (highs[-1]['level'] < highs[-2]['level'] and
                      lows[-1]['level'] < lows[-2]['level']):
                    return 'downtrend'
            return 'sideways'

        def find_inducement(points, current_idx, direction):
            """
            Tìm kiếm inducement (liquidity grab) trước khi xác nhận BOS/CHoCH

            direction: 'bullish' hoặc 'bearish'
            returns: dict với thông tin inducement hoặc None
            """
            if len(points) < 3:
                return None
            ...
            if direction == 'bullish':
                # Tìm Lower Low gần nhất (inducement cho bullish break)
                for i in range(current_idx - 1, -1, -1):
                    if points[i]['type'] == -1:  # Low point
                        # Kiểm tra xem có phải Lower Low không
                        for j in range(i - 1, -1, -1):
                            if points[j]['type'] == -1:
                                if points[i]['level'] < points[j]['level']:
                                    return {
                ...
                # Tìm Higher High gần nhất (inducement cho bearish break)
                for i in range(current_idx - 1, -1, -1):
                    if points[i]['type'] == 1:  # High point
                        # Kiểm tra xem có phải Higher High không
                        for j in range(i - 1, -1, -1):
                            if points[j]['type'] == 1:
                                if points[i]['level'] > points[j]['level']:
                                    return {
        ...
        def check_structure_break(ohlc_data, start_idx, level_to_break, direction, close_break):
            """Kiểm tra xem có break structure không"""
            if start_idx + 1 >= len(ohlc_data):
                return None

            for i in range(start_idx + 1, len(ohlc_data)):
                if direction == 'bullish':
                    price_to_check = ohlc_data["close"].iloc[i] if close_break else ohlc_data["high"].iloc[i]
                    if price_to_check > level_to_break:
                        return i
                else:  # bearish
                    price_to_check = ohlc_data["close"].iloc[i] if close_break else ohlc_data["low"].iloc[i]
                    if price_to_check < level_to_break:
                        return i
            return None

        # Main logic: Analyze each swing point for BOS/CHoCH
        for i in range(4, len(swing_points)):  # Cần ít nhất 4 swing points
            current_trend = detect_current_trend(swing_points, i)
            current_point = swing_points[i]

            # Tìm level cần break để tạo BOS/CHoCH
            if current_point['type'] == 1:  # Current point is High
                # Tìm Higher High trước đó để break (cho bullish BOS)
                for j in range(i - 1, -1, -1):
                    if swing_points[j]['type'] == 1:  # Previous high
                        target_level = swing_points[j]['level']

                        # Kiểm tra inducement
                        inducement = find_inducement(swing_points, i, 'bullish')

                        if inducement:
                            # Kiểm tra structure break
                            break_idx = check_structure_break(
                                ohlc, current_point['index'], target_level, 'bullish', close_break
                            )

                            if break_idx:
                                # Xác định BOS hay CHoCH
                                if current_trend == 'downtrend':
                                    # CHoCH: từ downtrend sang uptrend
                                    choch[current_point['index']] = 1
                                    level[current_point['index']] = target_level
                                    broken_index[current_point['index']] = break_idx
                                    inducement_level[current_point['index']] = inducement['level']
                                    inducement_index[current_point['index']] = inducement['index']
                                else:
                                    # BOS: continuation của uptrend
                                    # Chỉ là BOS nếu break ATH
                                    all_highs = [p['level'] for p in swing_points[:i + 1] if p['type'] == 1]
                                    if target_level == max(all_highs[:-1]):  # Break ATH
                                        bos[current_point['index']] = 1
                                        level[current_point['index']] = target_level
                                        broken_index[current_point['index']] = break_idx
                                        inducement_level[current_point['index']] = inducement['level']
                                        inducement_index[current_point['index']] = inducement['index']
            ...
            else:  # Current point is Low
                # Tìm Lower Low trước đó để break (cho bearish BOS)
                for j in range(i - 1, -1, -1):
                    if swing_points[j]['type'] == -1:  # Previous low
                        target_level = swing_points[j]['level']

                        # Kiểm tra inducement
                        inducement = find_inducement(swing_points, i, 'bearish')

                        if inducement:
                            # Kiểm tra structure break
                            break_idx = check_structure_break(
                                ohlc, current_point['index'], target_level, 'bearish', close_break
                            )

                            if break_idx:
                                # Xác định BOS hay CHoCH
                                if current_trend == 'uptrend':
                                    # CHoCH: từ uptrend sang downtrend
                                    choch[current_point['index']] = -1
                                    level[current_point['index']] = target_level
                                    broken_index[current_point['index']] = break_idx
                                    inducement_level[current_point['index']] = inducement['level']
                                    ...
                                    # BOS: continuation của downtrend
                                    # Chỉ là BOS nếu break ATL
                                    all_lows = [p['level'] for p in swing_points[:i + 1] if p['type'] == -1]
                                    if target_level == min(all_lows[:-1]):  # Break ATL
                                        bos[current_point['index']] = -1
                                        level[current_point['index']] = target_level
                                        broken_index[current_point['index']] = break_idx
                                        inducement_level[current_point['index']] = inducement['level']
        ...
        # Clean up: Remove overlapping signals
        active_signals = np.where(np.logical_or(bos != 0, choch != 0))[0]

        for i in range(len(active_signals)):
            current_idx = active_signals[i]
            current_break_idx = broken_index[current_idx]

            # Remove signals that were invalidated by later breaks
            for j in range(i + 1, len(active_signals)):
                later_idx = active_signals[j]
                later_break_idx = broken_index[later_idx]

                # If later signal breaks before current signal completes
                if later_break_idx < current_break_idx:
                    bos[current_idx] = 0
                    choch[current_idx] = 0
                    level[current_idx] = 0
                    broken_index[current_idx] = 0
                    inducement_level[current_idx] = 0
                    inducement_index[current_idx] = 0
                    break

        # Convert to pandas Series with NaN for zero values
        ...
        broken_index = pd.Series(np.where(broken_index != 0, broken_index, np.nan), name="BrokenIndex")
        inducement_level = pd.Series(np.where(inducement_level != 0, inducement_level, np.nan), name="InducementLevel")
        inducement_index = pd.Series(np.where(inducement_index != 0, inducement_index, np.nan), name="InducementIndex")

        # Để tương thích với version cũ, chỉ return 4 columns chính
        # Có thể uncomment 2 dòng cuối để có thêm thông tin inducement
        return pd.concat([
            bos,
            choch,
            level,
            broken_index,
            inducement_level,  # Uncomment để có thông tin inducement level
            inducement_index   # Uncomment để có thông tin inducement index
        ], axis=1)
