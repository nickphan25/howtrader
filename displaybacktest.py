...
class TradingViewGrid(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        # TradingView grid colors - màu sắc chính xác hơn
        light_grid_color = pg.QtGui.QColor(240, 243, 250, 80)  # Đường lưới nhạt nhất
        medium_grid_color = pg.QtGui.QColor(226, 232, 240, 120)  # Đường lưới trung bình
        major_grid_color = pg.QtGui.QColor(203, 213, 225, 180)  # Đường lưới chính
        ...
        # Draw horizontal grid lines - đường ngang
        y_step = (y_max - y_min) / y_lines
        for i in range(y_lines + 1):
            y = y_min + i * y_step
            if i % 5 == 0:
                # Đường chính - đậm và rõ hơn
                pen = pg.mkPen(major_grid_color, width=1.2, style=pg.QtCore.Qt.SolidLine)
            else:
                # Đường phụ - sử dụng màu medium
...
class CandlestickItem(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        for i, (timestamp, open_price, high_price, low_price, close_price, volume) in enumerate(self.data):
            # TradingView colors - màu sắc chính xác hơn
            is_bullish = close_price >= open_price

            if is_bullish:
                # Nến tăng - xanh lá TradingView
                pen_color = pg.QtGui.QColor(34, 171, 148)  # Viền
                fill_color = pg.QtGui.QColor(34, 171, 148)  # Nền đặc
                hollow_color = pg.QtGui.QColor(255, 255, 255)  # Nền rỗng
            else:
                # Nến giảm - đỏ TradingView
                pen_color = pg.QtGui.QColor(240, 67, 56)  # Viền
                fill_color = pg.QtGui.QColor(240, 67, 56)  # Nền đặc

            # Thiết lập pen cho đường wick
            wick_pen = pg.mkPen(pen_color, width=1.5, style=pg.QtCore.Qt.SolidLine)

            # Vẽ wick (đường râu) - đậm hơn
            ...
            if body_height > 0:
                ...
                # Tạo hình chữ nhật cho thân nến - rộng hơn
                candle_width = 0.6
                rect = pg.QtCore.QRectF(i - candle_width / 2, body_bottom, candle_width, body_height)

                # Thiết lập style cho thân nến
                body_pen = pg.mkPen(pen_color, width=1.8, style=pg.QtCore.Qt.SolidLine)
                p.setPen(body_pen)

                if is_bullish:
                    # Nến tăng - có thể làm rỗng hoặc đặc tùy setting
                    # Ở đây làm đặc như TradingView mặc định
...
class VolumeBarItem(pg.GraphicsObject):
    ...
    def generatePicture(self):
        ...
        for i, (timestamp, open_price, high_price, low_price, close_price, volume) in enumerate(self.data):
            ...
            # Chỉ 2 màu chính như TradingView
            if close_price >= open_price:
                color = pg.QtGui.QColor(34, 171, 148, 100)  # Xanh lá
            else:
                color = pg.QtGui.QColor(240, 67, 56, 100)   # Đỏ
...
class ExtendedTimeframeConverter:
    ...
    def convert_bars(self, bars, target_interval):
        """Convert bars to target timeframe"""
        if not bars:
            return []
        ...
        try:
            if target_interval == Interval.MINUTE:
                return bars  # Already 1-minute
            elif target_interval in [Interval.MINUTE_5, Interval.MINUTE_15, Interval.HOUR, Interval.HOUR_4]:
                return self._convert_standard_intervals(bars, target_interval)
                ...
                print(f"⚠️ Unsupported interval: {target_interval}")
                return bars
    ...
    def _convert_standard_intervals(self, minute_bars, target_interval):
        """Convert to standard intervals using simple aggregation"""
        interval_minutes = {
            Interval.MINUTE_5: 5,
        ...
        window_size = interval_minutes.get(target_interval, 1)
        result_bars = []

        # Group bars by time windows
        i = 0
        while i < len(minute_bars):
            # Start new window
            window_bars = []
            window_start_time = minute_bars[i].datetime

            # Collect bars for this window
            while (i < len(minute_bars) and
                   len(window_bars) < window_size):
                window_bars.append(minute_bars[i])
                i += 1

            if window_bars:
                # Create aggregated bar
                aggregated_bar = self._create_new_bar(window_bars[0], target_interval)

                # Update with all bars in window
                for bar in window_bars[1:]:
                    self._update_bar(aggregated_bar, bar)

                result_bars.append(aggregated_bar)
    ...
    def _update_bar(self, current_bar, new_bar):
        ...
        current_bar.datetime = new_bar.datetime  # Keep latest timestamp
...
class AdvancedHowtraderChart(QWidget):
    ...
    def setup_backtest_mode(self):
        """Setup widget for backtest mode"""
        try:
            # Hide/disable unnecessary controls
    ...
    def on_backtest_timeframe_changed(self):
        """Handle timeframe change in backtest mode"""
        try:
            ...
            # Convert timeframe using existing converter
            converter = ExtendedTimeframeConverter()
            converted_bars = converter.convert_bars(self.base_bars, new_interval)
    ...
    def _update_ohlc_data(self):
        """Update OHLC data arrays from current bars"""
        try:
            if not self.bars:
                return
    ...
    def _update_trade_markers(self):
        """Update trade markers for current timeframe"""
        try:
    ...
    def _find_closest_bar_index(self, target_time):
        """Find closest bar index for given timestamp"""
        try:
            min_diff = float('inf')
            closest_idx = None
    ...
    def load_data_async(self):
        """Skip data loading in backtest mode"""
        try:
            # Check if we're in backtest mode - CHỈ CẦN 1 LẦN CHECK
    ...
    def display_professional_chart(self):
        """Display professional trading chart with enhanced features"""
        try:
            ...
            # ✅ ADD TRADE MARKERS - USE CORRECT CHART REFERENCE
    ...
    def _display_trade_markers_simple(self):
        """Display trade markers with your preferred style"""
        try:
    ...
    def _convert_markers_to_indices(self):
        """Convert trade marker timestamps to chart indices"""
        try:
    ...
    def _display_trade_markers(self):
        """Display trade entry/exit markers on chart"""
        try:
            if not hasattr(self, 'trade_markers') or not self.trade_markers:
                print("⚠️ No trade markers to display")
                return

            if not hasattr(self, 'price_chart') or not self.price_chart:
                print("⚠️ No price chart to add markers")
                return

            # Group trades by entry/exit pairs
            entry_exits = self._group_trades_by_pairs()

            # Display markers
            for pair in entry_exits:
                self._add_trade_marker_pair(pair)

            print(f"✅ Trade markers displayed: {len(entry_exits)} pairs")

        except Exception as e:
            print(f"❌ Error displaying trade markers: {e}")

    def _group_trades_by_pairs(self):
        """Group trades into entry/exit pairs"""
        try:
            pairs = []
            ...
            for marker in self.trade_markers:
                ...
                is_entry = marker['offset'] in ['OPEN', 'open']

                if is_entry:
                    # Store entry trade
                    key = f"{marker['direction']}"  # Could be more complex
                    open_positions[key] = marker
                else:
                    # Find matching entry and create pair
                    key = f"{marker['direction']}"
    ...
    def _add_trade_marker_pair(self, pair):
        """Add entry/exit marker pair to chart - adapted to your style"""
        try:
            entry = pair['entry']
            exit_trade = pair['exit']
            direction = pair['direction']

            # Colors based on direction (your style)
            if direction == 'long':
                entry_color = 'green'
                exit_color = 'red'
            ...
            # Add entry marker (using index instead of timestamp)
            entry_scatter = pg.ScatterPlotItem(
                [entry['index']],  # ✅ Fixed: use index
                [entry['price']],
                size=15,
            ...
            self.price_widget.addItem(entry_scatter)  # ✅ Fixed: use price_widget

            # Add exit marker
            exit_scatter = pg.ScatterPlotItem(
                [exit_trade['index']],  # ✅ Fixed: use index
                [exit_trade['price']],
                size=15,
            ...
            self.price_widget.addItem(exit_scatter)  # ✅ Fixed: use price_widget

            # Add connecting line (your style)
            line_color = 'green' if exit_trade['price'] > entry['price'] else 'red'
            trade_line = pg.PlotDataItem(
                [entry['index'], exit_trade['index']],  # ✅ Fixed: use index
                [entry['price'], exit_trade['price']],
                pen=pg.mkPen(line_color, width=1, style=pg.QtCore.Qt.DashLine)
            )
            self.price_widget.addItem(trade_line)  # ✅ Fixed: use price_widget

            # Add profit/loss text (your style)
            ...
            text_item.setPos(exit_trade['index'], exit_trade['price'])  # ✅ Fixed: use index
            self.price_widget.addItem(text_item)  # ✅ Fixed: use price_widget

        except Exception as e:
            print(f"❌ Error adding trade marker pair: {e}")

    def _group_trades_into_pairs(self, marker_indices):
        """Group trade markers into entry-exit pairs"""
        try:
            trade_pairs = []
            entry_marker = None
